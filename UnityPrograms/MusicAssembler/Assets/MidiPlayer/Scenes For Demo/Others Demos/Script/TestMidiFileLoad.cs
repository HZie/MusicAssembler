    ...
    public class TestMidiFileLoad : MonoBehaviour
    {
        ...
        void OnGUI()
        {
            ...
            if (MidiLoaded != null)
            {
                ...
                {
                    infoEvents = new List<string>();
                    List<MPTKEvent> events = MidiLoaded.MPTK_ReadMidiEvents(StartTicks, EndTicks);
                    foreach (MPTKEvent evt in events)
                    {
                        switch (evt.Command)
                        {
                            case MPTKCommand.NoteOn:
                                infoEvents.Add(string.Format("NoteOn\tChannel:{0:00} \tTick:{1}\tNote:{2}\tDuration:{3,2}\tVelocity:{4}\n",
                                    evt.Channel, evt.Tick, evt.Value, Math.Round(evt.Duration, 2), evt.Velocity));
                                break;
                            case MPTKCommand.NoteOff:
                                infoEvents.Add(string.Format("NoteOff\tChannel:{0:00} \tTick:{1}\tNote:{2}\tDuration:{3,2}\tVelocity:{4}\n",
                                    evt.Channel, evt.Tick, evt.Value, Math.Round(evt.Duration, 2), evt.Velocity));
                                break;
                            case MPTKCommand.PatchChange:
                                infoEvents.Add(string.Format("Patch\tChannel:{0:00} \tTick:{1}\tPatch:{2}\n",
                                    evt.Channel, evt.Tick, evt.Value));
                                break;
                            case MPTKCommand.ControlChange:
                                infoEvents.Add(string.Format("Control\tChannel:{0:00} \tTick:{1}\tValue:{2}\tControler:{3}\n",
                                    evt.Channel, evt.Tick, evt.Value, evt.Controller));
                                break;
                            case MPTKCommand.MetaEvent:
                                if (!string.IsNullOrEmpty(evt.Info))
                                    infoEvents.Add(string.Format("Text\tChannel:{0:00} \tTick:{1}\tValue:{2}\n",
                                        evt.Channel, evt.Tick, evt.Info));
                                break;
                        }
                    }
                }
                ...
                StartTicks = (long)GUILayout.HorizontalSlider((float)StartTicks, 0f, (float)MidiLoaded.MPTK_TickLast, GUILayout.Width(buttonWidth));
                GUILayout.EndHorizontal();

                GUILayout.Space(10);
                GUILayout.BeginHorizontal();
                GUILayout.Label("Read Midi Events to: " + EndTicks, myStyle.TitleLabel3, GUILayout.Width(220));
                EndTicks = (long)GUILayout.HorizontalSlider((float)EndTicks, 0f, (float)MidiLoaded.MPTK_TickLast, GUILayout.Width(buttonWidth));
                GUILayout.EndHorizontal();

                GUILayout.Space(10);
                GUILayout.BeginHorizontal();
                GUILayout.Label("Duration in seconds: ", myStyle.TitleLabel3, GUILayout.Width(220));
                GUILayout.Label(MidiLoaded.MPTK_Duration.TotalSeconds.ToString(), myStyle.TitleLabel3);
                GUILayout.EndHorizontal();

                GUILayout.Space(10);
                GUILayout.BeginHorizontal();
                GUILayout.Label("Beat per Measure: ", myStyle.TitleLabel3, GUILayout.Width(220));
                GUILayout.Label(MidiLoaded.MPTK_NumberBeatsMeasure.ToString());
                GUILayout.EndHorizontal();

                GUILayout.Space(10);
                GUILayout.BeginHorizontal();
                GUILayout.Label("Quarter per Beat: ", myStyle.TitleLabel3, GUILayout.Width(220));
                GUILayout.Label(MidiLoaded.MPTK_NumberQuarterBeat.ToString(), myStyle.TitleLabel3);
                GUILayout.EndHorizontal();

                GUILayout.Space(10);
                GUILayout.BeginHorizontal();
                GUILayout.Label("MPTK_InitialTempo: ", myStyle.TitleLabel3, GUILayout.Width(220));
                GUILayout.Label(Convert.ToInt32( MidiLoaded.MPTK_InitialTempo).ToString(), myStyle.TitleLabel3);
                GUILayout.EndHorizontal();
