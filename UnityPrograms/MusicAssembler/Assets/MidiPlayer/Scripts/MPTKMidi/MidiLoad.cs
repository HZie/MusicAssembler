...
namespace MidiPlayerTK
{
    ...
    /// Usefull to load all the Midi events from a Midi.
    /// </summary>
    public class MidiLoad
    {
        ...
        public List<TrackMidiEvent> MidiSortedForNote;
        ...
        // 0 = guitar, 2 = piano
        public int guitarChannel = 0;
        public int pianoChannel = 2;
        ...
        private void Init()
        {
            ...
            // Channel Select
            //selectedChannel = guitarChannel;
            selectedChannel = pianoChannel;
        ...
        private void AnalyseMidi()
        {
            try
            {
                ...
                MidiSortedForNote = GetEventsForNote();

                ChangeTempo(MPTK_InitialTempo);

                 //foreach (TrackMidiEvent track in MidiSorted)
                 //{
                    //track.Event.AbsoluteTime += MPTK_DeltaTicksPerQuarterNote;
                    //Debug.Log(track.Event.AbsoluteTime);
                    //track.Event.AbsoluteTime += 480;    // one tick faster than original midi < for note preview
                    //Debug.Log(track.Event.AbsoluteTime);
                //}
        ...
        private List<TrackMidiEvent> GetEvents()
        {
            ...
            try
            {
                                        ...
                                        case MetaEventType.SetTempo:
                                            ...
                                            if (MPTK_MicrosecondsPerQuarterNote == 0)
                                            {
                                                if (MPTK_InitialTempo < 0)
                                                    MPTK_InitialTempo = ((TempoEvent)e).Tempo;
                ...
                // only this part is different; adding additional time to played midis
                foreach (TrackMidiEvent t in midievents)
                {
                    //t.Event.AbsoluteTime += MPTK_DeltaTicksPerQuarterNote * 2;
                   // Debug.Log(t.Event.AbsoluteTime);
                }

                if (midievents.Count > 0)
                    MPTK_TickLast = midievents[midievents.Count - 1].Event.AbsoluteTime;
                else
                    MPTK_TickLast = 0;
        ...
        private List<TrackMidiEvent> GetEventsForNote()
        {
            int countTracks = 0;
        ...
        private void CalculateDuration()
        {
            if (MidiSorted != null && MidiSorted.Count > 0)
            {
                ...
                if (LogEvents)
                    Debug.Log("     Duration :" + MPTK_Duration.TotalSeconds + " seconds");
            }
        }
        ...
        private bool ConvertToEvent(List<MPTKEvent> mptkEvents, TrackMidiEvent trackEvent)
        {
            ...
            switch (trackEvent.Event.CommandCode)
            {
                case MidiCommandCode.NoteOn:
                    ...
                    if (((NoteOnEvent)trackEvent.Event).OffEvent != null)
                    {
                        ...
                        if (noteon.NoteLength > 0)
                        {
                            ...
                            {
                                //Debug.Log(noteon.NoteNumber);
                                if(selectedChannel == pianoChannel)
                                    Kbd_KeyGenByOctave.instance.ShowMidiOn(noteon.NoteNumber);
                                else if(selectedChannel == guitarChannel)
                                    Str_FKeyGen.instance.ShowMidiOn(noteon.NoteNumber);
                        ...
                        else if (KeepNoteOff)
                        {
                            ...
                            //Kbd_PKeyGen.instance.ShowMidiNoteOff(midievent.Value);
                            Kbd_KeyGenByOctave.instance.ShowMidiOff(midievent.Value);
                            Str_FKeyGen.instance.ShowMidiOff(midievent.Value);
                ...
                case MidiCommandCode.NoteOff:
                    midievent = CreateNoteOff(mptkEvents, trackEvent);
                    // Debug.Log(((NoteOnEvent)trackEvent.Event).NoteNumber);
                    Kbd_KeyGenByOctave.instance.ShowMidiOff(((NoteOnEvent)trackEvent.Event).NoteNumber);
                    Str_FKeyGen.instance.ShowMidiOff(((NoteOnEvent)trackEvent.Event).NoteNumber);
        ...
        private MPTKEvent CreateNoteOff(List<MPTKEvent> mptkEvents, TrackMidiEvent trackEvent)
        {
            ...
            {
                if(selectedChannel== pianoChannel)
                    Kbd_KeyGenByOctave.instance.ShowMidiOff(noteoff.NoteNumber);
                if(selectedChannel== guitarChannel)
                    Str_FKeyGen.instance.ShowMidiOff(noteoff.NoteNumber);
