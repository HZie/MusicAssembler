    ...
    public class fluid_voice : MonoBehaviour
    {
        ...
        static int[] list_of_weakgenerators_to_initialize =
     {
        ...
        public void fluid_voice_start()
        {
            ...
            if (synth.VerboseVoice) Debug.LogFormat("   fluid_voice_start Audiosource volume:{0:0.000} loop:{1} pitch:{2:0.000} {3}", Audiosource.volume, Audiosource.loop, Audiosource.pitch, weakDevice ? "[WEAK]" : "");
        ...
        //private IEnumerator ThreadPlayNote()
        ...
        public void fluid_voice_write()
        {
            ...
            if (!Mathf.Approximately(Audiosource.volume, amp))
            {
                ...
                if (synth.VerboseVolume)
                    Debug.LogFormat("Volume [Id:{0:4}] {1} TimeDSP:{2:00000.000} Delta:{3:0.000} Att::{4,0:F2} modlfo_val:{5,0:F2} modlfo_to_vol:{6,0:F2} volenv_val:{7,0:F2} modenv_val:{8,0:F2} Amp:{9,0:F3} --> volume:{10,0:F3} {11}",
                        IdVoice, "", TimeFromStart, DeltaTimeWrite,
                        attenuation, modlfo_val, modlfo_to_vol, volenv_val, modenv_val, amp, Audiosource.volume, volenv_section);
            }
            //else
        ...
        public void fluid_weakvoice_write()
        {
            ...
            if (synth.VerboseEnvVolume)
                Debug.LogFormat("[Weak] Volume - [{0:4}] TimeDSP:{1:0.000} Delta:{2:0.000} synth.ReleaseTimeMin:{3:0.000} volenv_count:{4:0.000} Volume:{5:0.0000}",
