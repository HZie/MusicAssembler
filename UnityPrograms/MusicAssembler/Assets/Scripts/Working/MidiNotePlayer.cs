...
namespace MidiPlayerTK
{
    ...
    /// </summary>
    public class MidiNotePlayer : MidiPlayer
    {
        /// <summary>
        ...
        private MidiLoadForNote miditoplayForNote;
        ...
        protected IEnumerator<float> ThreadPlay(byte[] midiBytesToPlay = null)
        {
            ...
            try
            {
                ...
                miditoplayForNote = new MidiLoadForNote();
                ...
                miditoplayForNote.KeepNoteOff = MPTK_KeepNoteOff;
                miditoplayForNote.MPTK_Load(midiBytesToPlay);
            ...
            if (miditoplay != null)
            {
                ...
                try
                {
                   ...
                   // miditoplay.ChangeQuantization(MPTK_Quantization);
                    miditoplayForNote.ChangeSpeed(MPTK_Speed);
                    miditoplayForNote.ChangeQuantization(MPTK_Quantization);
                ...
                do
                {
                   ...
                   // List<MPTKEvent> midievents = miditoplay.ReadMidiEvents(timeFromStartPlay);
                    List<MPTKEvent> midieventsForNote = miditoplayForNote.ReadMidiEventsForNote(timeFromStartPlay);
                    ...
                    // Play notes read
                    if (midieventsForNote != null && midieventsForNote.Count > 0)
                    {
                        try
                        {
                            if (OnEventNotesMidi != null)
                            {
                                OnEventNotesMidi.Invoke(midieventsForNote);
                                //MPTK_Pause(1f);
                               // OnEventNotesMidi.Invoke(midievents);

                                //Kbd_PKeyGen.instance.ShowMidiNoteOn(60);
                                //Debug.Log(OnEventNotesMidi);
                            }
                        }
                        catch (System.Exception ex)
                        {
                        ...
                        //{
                        //    foreach (MPTKEvent midievent in midieventsForNote)
                        //    {
                        //        if (midievent.Command == MPTKCommand.MetaEvent && midievent.Meta == MPTKMeta.SetTempo && MPTK_EnableChangeTempo)
                        //        {
                        //            miditoplay.ChangeTempo(midievent.Duration);
